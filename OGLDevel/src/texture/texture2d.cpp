...
Texture2D::Texture2D(const char *imagePath, GLuint internalFormat, GLuint wrap_S, GLuint wrap_T)
{
    glGenTextures(1, &this->ID);
    this->Internal_Format = internalFormat;
    this->Wrap_S = wrap_S;
    ...
    if (image.getPixelizedData() == NULL) {
        ...
        this->Internal_Format = (image.getChannelCount() < 4) ? imageformat : this->Internal_Format;
    }
